package butterfly.music.fragment.musiclist;
...
import butterfly.music.activity.multichoice.MusicMultiChoiceActivity;
import butterfly.music.activity.multichoice.MultiChoiceStateHolder;
import butterfly.music.dialog.AddToMusicListDialog;
import butterfly.music.dialog.MessageDialog;
import butterfly.music.dialog.SingleChoiceDialog;
...
public abstract class BaseMusicListFragment extends Fragment {
    ...
    private void onMusicListItemClicked(int position) {
        if (matchPlaylistName() && matchPlaylistToken()) {
            mPlayerViewModel.getPlayerClient().playPause(position);
            return;
        }

        if (matchPlaylistName() && isPlayingMusic(position)) {
